...
level2Plat = tiled_map.get_object_by_name('Level_2')

level3Plat = tiled_map.get_object_by_name('Level_3')
...
door_img = pygame.transform.scale(door_img, (192, 64))
myObjects = []
...
while True:
    ...
    for layer in tiled_map.layers:
        if isinstance(layer, pytmx.TiledTileLayer) and (layer != objects) and (layer != doorDoors): # and (layer != collision):
            ...
            # for object in layer:
            #     if (object.type == "Key" and key_pickup == 1):
            #         SCREEN.blit(key,
        ...
        if playerrec.collidelistall(spaces):
            if yevar == 1:
                pos[1] = level2Plat.y -  CAMERA.x
                pos[0] = level2Plat.x - CAMERA.y
                yevar = 0
            elif yevar == 0:
                pos[1] = level3Plat.y + CAMERA.y - 2600
                pos[0] = level3Plat.x - CAMERA.x
                yevar = 3
        # check key with the current position of the player
        def checkkey(aplayerrec):
            for item in keyDoor:
                itemrec = pygame.Rect([item.x, item.y, item.width, item.height])
                if (itemrec.colliderect(playerrec)  and (item.type == "Key")):
                    throg = 0
        ...
        for object in myObjects:
            if (object.type == "Key" and object.visible == 1):
                SCREEN.blit(key,
                            [object.x - CAMERA.x + (SCREENWIDTH / 2),
                             cycle_key + object.y - CAMERA.y + (SCREENHEIGHT / 2)])
            for object in doorDoors:
                if (object.type == "door" and object.visible == 1):
                    SCREEN.blit(door_img,
                                [object.x - CAMERA.x + (SCREENWIDTH / 2),
                                 object.y - CAMERA.y + (SCREENHEIGHT / 2)])
            SCREEN.blit(player, [(SCREENWIDTH / 2), -25 + (SCREENHEIGHT / 2)])
